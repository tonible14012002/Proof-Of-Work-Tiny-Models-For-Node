...
export type TokenClassificationInputParams = {
  text: string;
  options?: {
...
type TokenClassificationResolver = (output: any) => void;

export const useInferenceTokenClassification = (modelId?: string) => {
  const { worker, runModel } = useWorkerContext();
  const [isPending, setIsPending] = useState(false);
  const [inferenceId, setInferenceId] = useState("");
  const waiterPromiseRef = useRef<{
    resolver: TokenClassificationResolver;
    rejecter: (reason?: any) => void;
  }>(null);

  const initWaiter = async () => {
    return new Promise((resolve: TokenClassificationResolver, reject) => {
  ...
  const classify = async (input: TokenClassificationInputParams) => {
    if (!modelId) {
      return;
    }
    ...
    try {
      ...
      console.error(e);
      toast("Error occurred while performing token classification", {
