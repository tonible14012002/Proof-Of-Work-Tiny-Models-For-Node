...
import type { SummarizerResult, WorkerMessage } from "@/schema/model";
import { useCallback, useEffect, useRef, useState } from "react";
import { toast } from "sonner";
import { v4 as uuidv4 } from "uuid";

export type SummarizerInputParams = {
  text: string;
  options?: any;
};

type SummarizerResolver = (_: SummarizerResult) => void;

export const useInferenceSummarizer = (modelId?: string) => {
  ...
  const initWaiter = async () => {
    return new Promise((resolve: SummarizerResolver, reject) => {
  ...
  const summarize = async (input: SummarizerInputParams) => {
    if (!modelId) {
      return;
    }
  ...
  const handleResponse = useCallback((e: MessageEvent) => {
