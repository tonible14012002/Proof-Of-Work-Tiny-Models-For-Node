...
import type { SentimentAnalysisResult, WorkerMessage } from "@/schema/model";
import { useCallback, useEffect, useRef, useState } from "react";
import { toast } from "sonner";
import { v4 as uuidv4 } from "uuid";

export type SentimentAnalysisInputParams = {
  text: string;
  options?: {
    top_k?: number | null;
  };
...
type AnalyserResolver = (_: SentimentAnalysisResult) => void;

export const useInferenceSentimentAnalysis = (modelId?: string) => {
  const { worker, runModel } = useWorkerContext();
  const [isPending, setIsPending] = useState(false);
  const [inferenceId, setInferenceId] = useState("");
  const waiterPromiseRef = useRef<{
    resolver: AnalyserResolver;
    rejecter: (reason?: any) => void;
  }>(null);

  const initWaiter = async () => {
    return new Promise((resolve: AnalyserResolver, reject) => {
  ...
  const analyze = async (input: SentimentAnalysisInputParams) => {
    if (!modelId) {
      return;
    }
    ...
    try {
      ...
      console.error(e);
      toast("Error occurred while summarizing", {
